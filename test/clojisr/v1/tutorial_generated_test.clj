...
(def var1 (def md (comp kindly/hide-code kind/md)))


(def var2 (md "# Clojisr tutorial"))
...
(def var171 (md "## Intermediary representation as Java objects."))
...
(def var173 (import (org.rosuda.REngine REXP REXPInteger REXPDouble)))


(def var174 (md "We can convert data between R and Java."))


(def var175 (-> "1:9" r r->java class))


(deftest test176 (is (= var175 REXPInteger)))


(def var177 (-> (REXPInteger. 1) java->r r->clj))


(deftest test178 (is (= var177 [1])))
...
(def var180 (-> "1:9" r r->java java->clj))


(deftest test181 (is (= var180 (range 1 10))))
...
(def var183 (-> (range 1 10) clj->java class))


(deftest test184 (is (= var183 REXPInteger)))


(def var185 (-> (range 1 10) clj->java java->clj))


(deftest test186 (is (= var185 (range 1 10))))
...
(def var188 (->> "1:9" r r->java java->native-clj))
...
(def var191 (-> "1+2" eval-r->java class))


(deftest test192 (is (= var191 REXPDouble)))


(def var193 (-> "1+2" eval-r->java (.asDoubles) vec))


(deftest test194 (is (= var193 [3.0])))


(def var195 (md "## More data conversion examples"))
...
(def var197 (-> "list(a=1:2,b='hi!')" r r->java java->clj))


(deftest test198 (is (= var197 {:a [1 2], :b ["hi!"]})))


(def var199 (md "Partially named lists are also supported"))


(def var200 (-> "list(a=1:2,'hi!')" r r->java java->clj))


(deftest test201 (is (= var200 {:a [1 2], 1 ["hi!"]})))
...
(deftest test205 (is (= var204 {:a [1 2], :b ["hi!"]})))
...
(def var207 (md "### Basic types convertion clj->r->clj"))


(def var208 (def clj->r->clj (comp r->clj r)))


(def var209 (clj->r->clj nil))


(deftest test210 (is (= var209 nil)))


(def var211 (clj->r->clj [10 11]))


(deftest test212 (is (= var211 [10 11])))


(def var213 (clj->r->clj [10.0 11.0]))


(deftest test214 (is (= var213 [10.0 11.0])))


(def var215 (clj->r->clj (list 10.0 11.0)))


(deftest test216 (is (= var215 [10.0 11.0])))


(def var217 (clj->r->clj {:a 1, :b 2}))


(deftest test218 (is (= var217 {:a [1], :b [2]})))


(def var219 (md "### Various R objects"))


(def var220 nil)


(def var221 (-> "list(a=1L,b=c(10,20),c='hi!')" r r->clj))


(deftest test222 (is (= var221 {:a [1], :b [10.0 20.0], :c ["hi!"]})))


(def var223 nil)


(def var224 (-> "c(10,20,30)" r r->clj))


(deftest test225 (is (= var224 [10.0 20.0 30.0])))


(def var226 nil)


(def var227 (-> "c(10L,20L,30L)" r r->clj))


(deftest test228 (is (= var227 [10 20 30])))


(def var229 nil)


(def var230 (-> 'euro r r->clj first))


(deftest test231 (is (= var230 13.7603)))


(def var232 nil)


(def var233 (-> r.stats/dnorm r.base/formals r->clj keys sort))


(deftest test234 (is (= var233 '(:log :mean :sd :x))))


(def var235 nil)


(def var236 (-> "NULL" r r->clj))


(deftest test237 (is (= var236 nil)))


(def var238 nil)


(def var239 (-> "TRUE" r r->clj))


(deftest test240 (is (= var239 [true])))


(def var241 (md "## Inspecting R functions"))
...
(def var243 (-> 'mean r.base/formals r->clj))


(deftest test244 (is (= var243 {:x (symbol ""), :... (symbol "")})))
...
(def var248 (-> 'mean.default r.base/formals r->clj))
...
(def var250 (md "## R-function-arglists"))
...
(def var252 (require-r '[base] '[stats] '[grDevices]))
